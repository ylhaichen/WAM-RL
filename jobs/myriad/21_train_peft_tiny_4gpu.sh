...
# Tiny four-GPU PEFT smoke test. This validates the Phase 3 update surface
# before any outcome-based RL objective is added.
...
SAVE_ROOT="${SAVE_ROOT:-${WAM_ROOT}/train_peft_tiny}"
MASTER_PORT="${MASTER_PORT:-29781}"
TRAINABLE_MODE="${TRAINABLE_MODE:-action_heads}"

export NUM_GPUS TRAIN_STEPS GRAD_ACCUM SAVE_ROOT MASTER_PORT TRAINABLE_MODE

print_job_context
echo "TRAINABLE_MODE=${TRAINABLE_MODE}"
...
export WAN_VA_TRAINABLE_MODE="${TRAINABLE_MODE}"

NGPU="${NUM_GPUS}" \
MASTER_PORT="${MASTER_PORT}" \
CONFIG_NAME=robotwin_peft_train \
