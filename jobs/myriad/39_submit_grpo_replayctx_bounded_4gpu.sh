...
REPLAY_CONTEXT_ESTIMATE_GB="${REPLAY_CONTEXT_ESTIMATE_GB:-7.25}"
...
export REPLAY_CONTEXT_ESTIMATE_GB CHECK_SCRATCH_HEADROOM SCRATCH_PATH MIN_SCRATCH_HEADROOM_GB DRY_RUN
...
if os.environ["CHECK_SCRATCH_HEADROOM"] == "1":
    ...
    required_gb = accepted_gb + min_headroom_gb
    ok = available_gb >= required_gb
    print(f"  scratch_path={scratch_path}")
    print(f"  scratch_available_gb={available_gb:.2f}")
    print(f"  required_for_accepted_plus_headroom_gb={required_gb:.2f}")
    print(f"  headroom_ok={str(ok).lower()}")
    if not dry_run and not ok:
        print(
            "Insufficient Scratch headroom for accepted replay-context estimate. "
