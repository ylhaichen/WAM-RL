...
GRPO_PROGRESS_EVERY="${GRPO_PROGRESS_EVERY:-50}"
...
GRPO_MAX_RESOLVED_GB="${GRPO_MAX_RESOLVED_GB:-0}"
...
export GRPO_TRAINABLE_MODE GRPO_CONFIG_NAME GRPO_ACTION_NUM_INFERENCE_STEPS GRPO_LOGPROB_REDUCTION GRPO_LOGPROB_STD_FLOOR GRPO_PROGRESS_EVERY
export GRPO_STORAGE_AUDIT_JSON GRPO_AUDIT_REPLAY_CONTEXTS GRPO_MAX_RESOLVED_GB
...
STD_FLOOR_ARGS=()
if [ -n "${GRPO_LOGPROB_STD_FLOOR}" ]; then
    STD_FLOOR_ARGS=(--logprob-std-floor "${GRPO_LOGPROB_STD_FLOOR}")
fi
...
STORAGE_AUDIT_ARGS=(
    "${GRPO_GROUPS_PATH}"
    --out-json "${GRPO_STORAGE_AUDIT_JSON}"
    --fail-on-missing
    --print-summary
)
case "${GRPO_AUDIT_REPLAY_CONTEXTS}" in
    true|True|1|yes|YES|on|ON)
        STORAGE_AUDIT_ARGS+=(--inspect-replay-contexts --omit-replay-context-mapping)
        ;;
esac
if [ "${GRPO_MAX_RESOLVED_GB}" != "0" ] && [ -n "${GRPO_MAX_RESOLVED_GB}" ]; then
    STORAGE_AUDIT_ARGS+=(--max-resolved-gb "${GRPO_MAX_RESOLVED_GB}")
fi

python tools/audit_grpo_artifact_storage.py "${STORAGE_AUDIT_ARGS[@]}"
...
python tools/train_actor_replay_grpo.py \
    ...
    --logprob-reduction "${GRPO_LOGPROB_REDUCTION}" \
    "${STD_FLOOR_ARGS[@]}" \
    --progress-every "${GRPO_PROGRESS_EVERY}" \
    "${ACTION_STEPS_ARGS[@]}"
