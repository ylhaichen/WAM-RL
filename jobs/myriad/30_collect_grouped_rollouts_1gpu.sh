...
# Direct workstation runner for one-GPU grouped rollout collection.
# This intentionally does not use a batch scheduler or Apptainer resource requests.
...
if [ "${NUM_GPUS}" -ne 1 ]; then
    echo "This direct runner supports exactly one GPU. Got NUM_GPUS=${NUM_GPUS}." >&2
    exit 2
fi
...
export REPO_ROOT WAM_ROOT WAN_VA_MODEL_PATH ROBOTWIN_ROOT WAN_VA_ROBOTWIN_ROOT
...
export ACTION_NUM_INFERENCE_STEPS RUN_ID TOKENIZERS_PARALLELISM
